<table class="table table-sm table-hover table-striped mt-3">
    ...
    <tbody>
        <tr>
            ...
            <th style="width: 20pt; font-weight:bold;" class="text-primary">{{ number_format($saldoSebelum) }}</th>
        </tr>
        @php
            $saldo = $saldoSebelum;
        @endphp
    ...
    <tfoot>
        <tr class="bg-light">
            ...
            <th style="width: 20pt; font-weight:bold;" class="text-primary">{{ $kasPendaftaran->sum('kredit') }}
            </th>
            <th style="width: 20pt; font-weight:bold;" class="text-primary">{{ $kasPendaftaran->sum('debet') }}</th>
            <th style="width: 20pt; font-weight:bold;" class="text-primary">
                {{ $saldo }}</th>
