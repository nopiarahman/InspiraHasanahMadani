        ...
        @foreach ($DPtertunggak as $tunggakan)
            @if ($tunggakan->sisaDp - $tunggakan->potonganDp > 0)
                ...
                <tr>
                    ...
                    <td data-order="{{ tempoDpNunggak($tunggakan, $start)->tempo }}"><a class="text-danger"
                    ...
                    @else
                        <td data-order="{{ bulanDpTunggakanBerjalan(tempoDpNunggak($tunggakan, $start), $start) }}">
                            {{ bulanDpTunggakanBerjalan(tempoDpNunggak($tunggakan, $start), $start) }}
                        </td>
                    @endif
                </tr>
            @endif
            ...
            @if ($tunggakan)
                <tr>
                    ...
                    @if (bulanCicilanTunggakanBerjalan(tempoCicilanNunggak($tunggakan, $start), $start) > $tunggakan->sisaKewajiban - cicilanTerbayar($tunggakan->id, $start))
                        <td data-order="{{ $tunggakan->sisaKewajiban - cicilanTerbayar($tunggakan->id, $start) }}">
                            {{ $tunggakan->sisaKewajiban - cicilanTerbayar($tunggakan->id, $start) }}
                        </td>
                    @else
                        <td
                            data-order="{{ bulanCicilanTunggakanBerjalan(tempoCicilanNunggak($tunggakan, $start), $start) }}">
                            {{ bulanCicilanTunggakanBerjalan(tempoCicilanNunggak($tunggakan, $start), $start) }}
                        </td>
                    @endif
                </tr>
            @endif
