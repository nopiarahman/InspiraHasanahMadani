...
@section('content')
    <div class="section-header">
        <h1>Cetak Kwitansi Tambahan</h1>
        @if (auth()->user()->role == 'admin' || auth()->user()->role == 'projectmanager' || auth()->user()->role == 'kasir')
            <div class="kanan">
                <button onclick="cetak('wrapper')" class="btn btn-primary"> <i class="fas fa-print fa-L"></i> Cetak
                    Kwitansi</button>
                <a type="button" href="{{ route('cetakKwitansiTambahanPDF', ['id' => $id->id]) }}"
                    class="btn btn-info ml-2"> <i class="fas fa-file-pdf    "></i> Cetak PDF</a>
            </div>
        @endif
    </div>
            ...
            <section class="invoice ">
                        ...
                        <div class="page-header">
                            <div class="row">
                                ...
                                <div class="col-6 ml-n4">
                                    ...
                                    <small style="font-size: medium"> <i class="fa fa-phone" aria-hidden="true"></i> :
                                        {{ $proyek->telpPT }}</small><br>
                                    <small style="font-size: medium"> <i class="fas fa-envelope    "></i> :
                                        {{ $proyek->emailPT }}</small>
                                </div>
                                <div class="col-3 ">
                                    <small style="font-size: medium">Nomor Faktur:
                                        ...
                                        @else
                                            CB
                                        @endif
                                        {{ romawi(Carbon\Carbon::parse($id->tanggal)->isoFormat('MM')) }}/{{ $id->ke }}
                                    </small><br>
                                    <small style="font-size: medium">Tanggal: {{ formatTanggal($id->tanggal) }} </small>
                                    <br>
                                    <small style="font-size: medium">Kode Pelanggan:
                                        C{{ $pembelian->kavling->blok }}</small>
                                </div>
                            </div>
                            </h2>
                        </div>
                ...
                <table class="table table-sm table-responsive" style="font-size: medium">
                    <tr style="font-size: medium">
                        ...
                        <td style="width: 25%">: <span class="font-weight-bold">{{ $pembelian->pelanggan->nama }}</span>
                        </td>
                        <td></td>
                        <th rowspan="2" style="width: 65%;vertical-align:middle" class="border border-success">
                            <h5 class="text-center align-middle mb-n1" style="font-style: italic">
                                {{ terbilang($id->jumlah) }} Rupiah</h5>
                        </th>
                    </tr>
                    <tr>
                        <td>Sejumlah Uang</td>
                    ...
                    <div class="col-6">
                        <span class="lead font-weight-bold">Metode Pembayaran:
                            @if ($id->sumber == 'Cash' || $id->sumber == 'cash')
                                <span class="text-primary">TUNAI</span>
                        </span>
                    @else
                        ...
                        <table class="" style="border-collapse: collapse">
                            <tr class="px-n1">
                                ...
                                <td>: <span class="text-warning"> Rp
                                        {{ number_format($id->tambahan->total) }}</span>
                                </td>
                            </tr>
                            <th>Total Terbayar</th>
                            <td>: <span
                                    class="text-warning">Rp.{{ number_format(hitungDetailTambahan($id->tambahan->id)) }}
                            </td>
                ...
                <table class="table table-sm table-responsive" style="font-size: medium">
                    <tr style="font-size: medium">
                        ...
                        <td style="width: 25%">: <span
                                class="font-weight-bold">{{ $pembelian->pelanggan->nama }}</span>
                        </td>
                        <td></td>
                        <th rowspan="2" style="width: 65%;vertical-align:middle" class="border border-success">
                            <h5 class="text-center align-middle mb-n1" style="font-style: italic">
