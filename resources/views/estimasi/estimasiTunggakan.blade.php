                    ...
                    @foreach ($DPtertunggak as $tunggakan)
                        @if ($tunggakan->sisaDp - $tunggakan->potonganDp > 0)
                            ...
                            <tr>
                                ...
                                <td>{{ $n, $n++ }}</td>
                                <td><a
                                        href="{{ route('pelangganDetail', ['id' => $tunggakan->pelanggan->id]) }}">{{ $tunggakan->pelanggan->nama }}</a>
                                </td>
                                ...
                                <td data-order="{{ tempoDpNunggak($tunggakan, $start)->tempo }}"><a
                                        class="text-danger"
                                        href="{{ route('DPKavlingTambah', ['id' => $tunggakan->id]) }}">
                                        {{-- <td> --}}
                                        1-10
                                        {{ Carbon\Carbon::parse(tempoDpNunggak($tunggakan, $start)->tempo)->isoFormat('MMMM YYYY') }}
                                    </a>
                                </td>
                                @if (bulanDpTunggakanBerjalan(tempoDpNunggak($tunggakan, $start), $start) > $tunggakan->dp - dpTerbayar($tunggakan->id, $start))
                                    <td data-order="{{ $tunggakan->dp - dpTerbayar($tunggakan->id, $start) }}">
                                        Rp.
                                        {{ number_format($tunggakan->dp - dpTerbayar($tunggakan->id, $start)) }}
                                    </td>
                                @else
                                    <td
                                        data-order="{{ bulanDpTunggakanBerjalan(tempoDpNunggak($tunggakan, $start), $start) }}">
                                        Rp.
                                        {{ number_format(bulanDpTunggakanBerjalan(tempoDpNunggak($tunggakan, $start), $start)) }}
                                    </td>
                                @endif
                                <td>
                                    {{ $tunggakan->dp - dpTerbayar($tunggakan->id, $start) }}
                                </td>
                            </tr>
                        @endif
                    @endforeach
                        ...
                        @if ($tunggakan)
                            <tr>
                                ...
                                <td data-order="{{ tempoCicilanNunggak($tunggakan, $start)->tempo }}">
                                    <a class="text-danger"
                                        href="{{ route('unitKavlingDetail', ['id' => $tunggakan->id]) }}">
                                        1-10
                                        {{ Carbon\Carbon::parse(tempoCicilanNunggak($tunggakan, $start)->tempo)->isoFormat('MMMM YYYY') }}

                                    </a>
                                </td>
                                @if (bulanCicilanTunggakanBerjalan(tempoCicilanNunggak($tunggakan, $start), $start) > $tunggakan->sisaKewajiban - cicilanTerbayar($tunggakan->id, $start))
                                    <td
                                        data-order="{{ $tunggakan->sisaKewajiban - cicilanTerbayar($tunggakan->id, $start) }}">
                                        Rp.
                                        {{ number_format($tunggakan->sisaKewajiban - cicilanTerbayar($tunggakan->id, $start)) }}
                                    </td>
                                @else
                                    <td
                                        data-order="{{ bulanCicilanTunggakanBerjalan(tempoCicilanNunggak($tunggakan, $start), $start) }}">
                                        Rp.
                                        {{ number_format(bulanCicilanTunggakanBerjalan(tempoCicilanNunggak($tunggakan, $start), $start)) }}
                                    </td>
                                @endif
                            </tr>
                        @endif
